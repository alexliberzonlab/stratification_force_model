...
if flag ==5;load('Data/TrajPF_complete.mat');
    ...
    TRAJP =  eval(['TrajPF']); 
    lam=0.25;             % hyperbolic function constructive param used to simulate density profile
end

Ind_vec = [1:length(TRAJP)];                        % Vector containing indexes of all trajectories
options = odeset('reltol', 1e-6, 'abstol', 1e-6);   % ODE integration options
...
for i=1:length(Ind_vec)
    ...
    rhop   = TRAJP(i).rhop;             % particle density              [kg/m3]
    d      = TRAJP(i).d;                % particle diameter             [m]
    z_exp  = TRAJP(i).z;                % particle z-position           [m]
    vz_exp = TRAJP(i).vz;               % particle vertical velocity    [m/s]
    t_exp  = TRAJP(i).time;             % time                          [s]
    t0= t_exp(1);                       % initial time                  [s]
    z0= z_exp(1);                       % particle initial position     [m]
